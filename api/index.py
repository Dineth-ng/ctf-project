...
app = FastAPI(
    ...
    description="FastAPI Backend for Vercel and Local Deployment",
    version="1.2.0"
)
...
# Determine paths (handle Vercel serverless /tmp environment)
...
# In-memory fallback
in_memory_scoreboard: Dict[str, list] = {"teams": []}
...
def load_scoreboard() -> dict:
    ...
    return in_memory_scoreboard

def save_scoreboard(data: dict):
    global in_memory_scoreboard
    in_memory_scoreboard = data
    try:
        with open(SCOREBOARD_FILE, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
    except Exception as e:
        print(f"[!] Warning: Could not write to disk (using memory): {e}")
...
def verify_flag(payload: FlagVerifyRequest):
    ...
    if submitted_flag == expected_flag:
        ...
        if stage_key not in team_entry["stage_times"]:
            ...
            save_scoreboard(scoreboard)
            print(f"[+] Flag Solved: Stage {stage_id} by '{team_name}' at {time_str}")
...
# Serve static files locally (when not running under Vercel serverless)
if not os.environ.get("VERCEL") and os.path.exists(PUBLIC_DIR):
